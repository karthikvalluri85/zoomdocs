...
- The average meeting duration is found by averaging the duration across
  all meetings.

- The active user count is the number of unique users who have hosted at
  least one meeting.
...
- A Meeting_ID in Attendees or Features_Usage must exist in the Meetings
  table.
...
- The ticket volume by type shows how many tickets were created for each
  type of issue.
...
- Usage correlation with billing events (e.g., users who upgrade after a
  certain usage threshold).
...
- Data must be validated against schema constraints (e.g., valid dates,
